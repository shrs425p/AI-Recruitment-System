...
def test_upload_rejects_unsupported_files_and_preserves_duplicates(tmp_path, monkeypatch):
    from io import BytesIO

    resumes_dir = tmp_path / "resumes"
    resumes_dir.mkdir()
    (resumes_dir / "resume.pdf").write_bytes(b"original")
    monkeypatch.setattr(upload_routes, "RESUMES_FOLDER", resumes_dir)
    ...
    response = client.post(
        ...
        data={
            "resumes": [
                (BytesIO(b"new resume"), "resume.pdf"),
                (BytesIO(b"not allowed"), "resume.exe"),
            ]
        },
    ...
    assert data["saved"] == ["resume_1.pdf"]
    assert data["count"] == 1
    assert len(data["rejected"]) == 1
    assert (resumes_dir / "resume.pdf").read_bytes() == b"original"
    assert (resumes_dir / "resume_1.pdf").read_bytes() == b"new resume"
