...
def _run_txt_stage(results):
    _set_auto_pipeline_status(
        "running", f"Step 1/{AUTO_PIPELINE_STEPS}: Converting resumes to plain text...",
        started=time.time(), step="txt", result=results
    )
    input_path = RESUMES_FOLDER
    output_txt_path = OUTPUT_FOLDER / "txt"
    output_txt_path.mkdir(parents=True, exist_ok=True)
    pdf_files = [f for f in input_path.iterdir() if f.is_file() and f.suffix.lower() in [".pdf", ".png", ".jpg", ".jpeg"]]
...
def _run_nlp_stage(results):
    _set_auto_pipeline_status(
        "running", f"Step 2/{AUTO_PIPELINE_STEPS}: Extracting structured candidate profiles...",
        step="nlp", result=results
    )
    output_txt_path = OUTPUT_FOLDER / "txt"
...
def _run_ranking_stage(results):
    _set_auto_pipeline_status(
        "running", f"Step 3/{AUTO_PIPELINE_STEPS}: Updating ranking from the active job description...",
    ...
    if latest_ranking and "job_description" in latest_ranking:
        ...
        if candidates:
            ...
            ranked_filenames = {c.get("_source_file") for c in ranked_candidates if c.get("_source_file")}
            new_candidates = [c for c in candidates if c.get("_source_file") not in ranked_filenames]
            if new_candidates:
                scored_new = []
                ...
                results["ranking"] = {
                    "success": True, "message": f"Evaluated {len(scored_new)} new candidate(s).", "count": len(scored_new),
                    "shortlist_report": shortlist.get("files", {})
                }
            else:
                results["ranking"] = {"success": True, "message": "All candidate profiles are already ranked.", "count": 0}
        else:
             results["ranking"] = {"success": True, "message": "All candidate profiles are already ranked.", "count": 0}

def _run_scheduling_stage(results):
    _set_auto_pipeline_status(
        "running", f"Step 4/{AUTO_PIPELINE_STEPS}: Scheduling ranked candidates and creating tokens...",
        step="scheduling", result=results
    )
    output_ranking_path = OUTPUT_FOLDER / "ranking"
    ...
    if not top_candidates:
        results["scheduling"] = {"success": False, "message": "No ranked candidates available to schedule.", "count": 0}
        return
    ...
    if not slots_raw:
        now = datetime.now()
        slots_raw = [f"{(now + timedelta(days=i)).strftime('%Y-%m-%d')} 10:00" for i in range(1, 6)]
    ...
    if hr_slots:
        ...
        new_candidates = [c for c in top_candidates if (c.get("candidate_name") or c.get("candidate", "Unknown")) not in scheduled_names]
        ...
        if new_candidates:
            ...
            for i, candidate in enumerate(new_candidates):
                ...
                new_scheduled.append({
                    "rank": start_offset + i + 1, "candidate_name": name, "source_file": source,
                    "score": score, "offered_slots": slot_strings, "selected_slot": slot_strings[0] if slot_strings else None,
            ...
            sched_run_id = create_run("scheduling", {"job_title": job_title, "count": len(new_scheduled), "source": "auto_pipeline"})
            ...
            for entry in new_scheduled:
                if entry["status"] == "CONFIRMED":
                    ...
                    create_interview_token(
                        token=token, candidate_name=entry["candidate_name"], source_file=entry["source_file"],
                        job_title=job_title, rank=entry["rank"], score=entry["score"]
                    )

            results["scheduling"] = {"success": True, "message": f"Coordinated {len(new_scheduled)} new candidate(s).", "count": len(new_scheduled)}
        else:
            results["scheduling"] = {"success": True, "message": "All ranked candidates are already scheduled.", "count": 0}

def _run_reports_stage(results):
    _set_auto_pipeline_status(
        "running", f"Step 5/{AUTO_PIPELINE_STEPS}: Generating reports for completed interviews...",
        step="reports", result=results
    )
    ...
    if transcripts:
        ...
        new_transcripts = [t for t in transcripts if t.get("_source_file") not in existing_report_sources]
        all_reports = []
        success_count = 0
        ...
        results["reports"] = {"success": True, "message": f"Generated {success_count} new report(s).", "count": success_count}
    else:
        results["reports"] = {"success": False, "message": "No completed interviews yet. Reports will run automatically after interviews exist.", "count": 0}
...
def _run_auto_pipeline_job():
    ...
    try:
        ...
        _set_auto_pipeline_status(
            "done", "Auto-pipeline completed.", finished=time.time(), step="complete", result=results
        )
    except Exception as ex:
        _set_auto_pipeline_status(
            "error", f"Auto-pipeline failed: {ex}", finished=time.time(), error=str(ex), result=results
        )
...
def register_dashboard_routes(app):
    ...
    def api_open_output_folder():
        try:
            folder_path = open_folder(OUTPUT_FOLDER)
            return jsonify({"success": True, "path": folder_path})
        except Exception as e:
            logger.exception("Could not open output folder: %s", OUTPUT_FOLDER)
