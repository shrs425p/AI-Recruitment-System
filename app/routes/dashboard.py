...
from app.core import RESUMES_FOLDER, OUTPUT_FOLDER, pipeline_tasks, _save_tasks
from app.database import create_run, finish_run, upsert_candidate, save_schedule as db_save_schedule, create_interview_token
...
from ranking_engine import load_candidates, score_candidate, save_leaderboard_txt, save_scores_json
...
def register_dashboard_routes(app):
    ...
    def api_run_auto_pipeline():
        pipeline_tasks["auto_pipeline"] = {"status": "running", "started": time.time(), "message": "Initiating Safe Auto-Pilot..."}
        _save_tasks()
        ...
        try:
            # ── Step 1: PDF to TXT ──
            pipeline_tasks["auto_pipeline"]["message"] = "Step 1/4: Converting resumes to plain text..."
            _save_tasks()
            
            input_path  = RESUMES_FOLDER
            output_txt_path = OUTPUT_FOLDER / "txt"
            output_txt_path.mkdir(parents=True, exist_ok=True)
            pdf_files = [f for f in input_path.iterdir()
                         if f.is_file() and f.suffix.lower() in [".pdf", ".png", ".jpg", ".jpeg"]]
            ...
            pipeline_tasks["auto_pipeline"]["message"] = "Step 2/4: Performing AI NLP parsing on text resumes..."
            _save_tasks()
            ...
            # ── Step 3: Incremental Leaderboard Ranking ──
            pipeline_tasks["auto_pipeline"]["message"] = "Step 3/4: Safe incremental job description ranking..."
            _save_tasks()
                ...
                if latest_ranking and "job_description" in latest_ranking:
                    ...
                    if candidates:
                        ...
                        if new_candidates:
                            ...
                            rank_run_id = create_run("ranking", {"job_title": jd_data.get("job_title"), "count": len(ranked)})
                            for r in ranked:
                                upsert_candidate(
                                    run_id=rank_run_id,
                            ...
                            results["ranking"] = {
                                "success": True,
                                "message": f"Successfully evaluated {len(scored_new)} new candidate(s).",
                                "count": len(scored_new)
                            }
                        else:
                            results["ranking"] = {
                                "success": True,
                                "message": "All candidate profiles are already evaluated against Job Description.",
                                "count": 0
                            }
            else:
                results["ranking"] = {
                    "success": False,
                    "message": "No active Job Description set up. Paste a Job Description under Ranking tab to enable ranking.",
                    "count": 0
                }

            # ── Step 4: Incremental Scheduling ──
            pipeline_tasks["auto_pipeline"]["message"] = "Step 4/4: Safe scheduling and token generation..."
            _save_tasks()
            ...
            if top_candidates:
                ...
                if not slots_raw:
                    ...
                    for i in range(1, 6):
                        ...
                        default_slots.append(f"{future_day.strftime('%Y-%m-%d')} 10:00")
                    slots_raw = default_slots
                    ...
                    if new_candidates:
                        ...
                        for i, candidate in enumerate(new_candidates):
                            ...
                            new_scheduled.append({
                                ...
                                "selected_slot":  [s.strftime("%Y-%m-%d %H:%M") for s in offered_slots][0] if offered_slots else None,
                        ...
                        sched_run_id = create_run("scheduling", {"job_title": job_title, "count": len(new_scheduled)})
                        ...
                        results["scheduling"] = {
                            "success": True,
                            "message": f"Successfully coordinated slots for {len(new_scheduled)} new candidate(s).",
            ...
            pipeline_tasks["auto_pipeline"] = {"status": "done", "result": results, "message": "Pipeline completed successfully!"}
            _save_tasks()
            return jsonify({"success": True, "result": results})

        except Exception as ex:
            pipeline_tasks["auto_pipeline"] = {"status": "error", "error": str(ex), "message": f"Failed: {str(ex)}"}
            _save_tasks()
