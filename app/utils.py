...
def _repair_json_string(s: str) -> str:
    """Repair common JSON formatting defects like trailing commas."""
...
def clean_json_response(text: str) -> dict:
    ...
    LLMs often wrap JSON in ```json ... ``` blocks or emit trailing commas.
    This function:
      1. Removes ```json and ``` markers.
      2. Tries json.loads() on the cleaned string (with trailing comma repair).
      3. If that fails, finds the first '{' and last '}' and retries.
    ...
    text = re.sub(r"```json", "", text, flags=re.IGNORECASE)  # remove opening ```json
    ...
    try:
        ...
        try:
            # Step 2b: Retry direct parse with trailing comma repair
