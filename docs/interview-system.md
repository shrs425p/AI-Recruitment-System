...
Candidate APIs are also session-key protected. Opening a token creates an interview session with a private per-session key bound to the same client address and browser user agent. Answer submission, finish, browser flags, and webcam frame analysis are rejected if the request has only a guessed `session_id` or comes from another device on the same network.
