...
class WebcamProctor:
    ...
    def __init__(self, session_id: str):
        self.session_id: str = session_id   # unique identifier for this interview session
        self.active: bool = False           # set to True when proctoring is running
        self.started_at: Any = None         # start timestamp
        self.cap: Any = None                # OpenCV VideoCapture object
        self._detector: Any = None          # MediaPipe FaceDetection or CascadeClassifier
        self._thread: Any = None            # background proctoring thread
        self._lock: threading.Lock = threading.Lock()  # protects shared state

        # Shared state (safe to read from any thread via get_frame / get_status)
        self.latest_frame_b64: Any = None   # most recent webcam frame encoded as base64 JPEG
        self.face_count: int = 0            # number of faces in the most recent frame
        self.flags: list = []               # list of flag event dicts recorded so far
