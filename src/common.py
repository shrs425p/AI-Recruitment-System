...
def data_path(relative: str) -> Path:
    """Return a path to a *file* inside the app data directory, creating parent dirs."""
    path = APP_DATA_DIR / "data" / relative
    path.parent.mkdir(parents=True, exist_ok=True)
    return path


def data_dir(relative: str) -> Path:
    """Return a path to a *directory* inside the app data directory, creating it if needed."""
    path = APP_DATA_DIR / "data" / relative
...
def call_ollama(system_msg: str, user_msg: str, temperature: float = 0.0, num_predict: int = 2048) -> str:
    ...
    if in_loop:
        ...
        t.join()
        if exception:
            logger.error(f"  [ERROR] AI call failed: {exception}")
            raise exception
    else:
        try:
            result = asyncio.run(call_ai_async(system_msg, user_msg, temperature, num_predict))
